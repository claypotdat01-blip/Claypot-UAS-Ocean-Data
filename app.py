...
============================================================
Real-Time  : CMEMS · NASA MODIS/ERDDAP · ERA5/Open-Meteo · BMKG/Open-Meteo
...
from data_fetcher import (
    build_realtime_dataframe, hitung_indeks, get_fsi_status, get_ohi_status,
    derivasi_do_ph, CHLA_MUSIMAN, FSI_KLASIFIKASI, OHI_KLASIFIKASI, RANGE_REF,
)
...
.data-note-derive { font-family: 'JetBrains Mono', monospace; font-size: 10px; color: #5A3080; background: #F3EEFB; border-left: 3px solid #8B5CF6; padding: 8px 12px; border-radius: 0 4px 4px 0; margin-top: 4px; }
...
.fsi-bar-wrap { background:#E8F0F8; border-radius:6px; height:10px; margin:4px 0 8px; overflow:hidden; }
.fsi-legend-row { display:flex; gap:6px; flex-wrap:wrap; margin-top:6px; }
.fsi-legend-chip { font-family:'JetBrains Mono',monospace; font-size:9px; padding:2px 8px; border-radius:3px; border:1px solid; display:inline-block; }
</style>
"""
...
# Hitung OHI dan FSI dengan normalisasi rentang tetap (konsisten dengan RT)
def _hitung_indeks_df_row(row):
    ohi, fsi = hitung_indeks(
        chla=row["chla"], do_=row["do"], ph=row["ph"],
        salinitas=row["salinitas"], gelombang=row["gelombang"],
        current_speed=row["current_speed"],
    )
    return pd.Series({"Ocean_Health_Index": ohi, "Fisheries_Index": fsi})

idx_df = df.apply(_hitung_indeks_df_row, axis=1)
df["Ocean_Health_Index"] = idx_df["Ocean_Health_Index"]
df["Fisheries_Index"]    = idx_df["Fisheries_Index"]
...
def render_map(df_map, z_col, colorscale, height=520):
    ...
    if geojson_prov:
        map_layers.append(dict(sourcetype="geojson", source=geojson_prov,
    ...
    fig.update_layout(
        ...
        coloraxis_colorbar=dict(thickness=12, len=0.65,
...
# =========================================
# HELPER: ARAH ARUS & ANGIN
# =========================================
def get_arah_arus(df_src):
    if df_src is None or (hasattr(df_src, "empty") and df_src.empty):
        return "tidak diketahui", "?"
    mean_uo = float(df_src["uo"].mean()); mean_vo = float(df_src["vo"].mean())
    angle_nav = (90.0 - np.degrees(np.arctan2(mean_vo, mean_uo)) + 360.0) % 360.0
    arah_list = [("Utara","↑"),("Utara-Timur Laut","↗"),("Timur Laut","↗"),("Timur-Timur Laut","↗"),
    ...
    return arah_list[int((angle_nav + 11.25) / 22.5) % 16]

def get_arah_angin(df_src):
    if df_src is None or (hasattr(df_src, "empty") and df_src.empty):
        return "tidak diketahui", "?"
    mean_u = float(df_src["angin_u"].mean()); mean_v = float(df_src["angin_v"].mean())
    angle_nav = (90.0 - np.degrees(np.arctan2(-mean_v, -mean_u)) + 360.0) % 360.0
    arah_list = [("Utara","↓"),("Utara-Timur Laut","↙"),("Timur Laut","↙"),("Timur-Timur Laut","↙"),
        ("Timur","←"),("Timur-Tenggara","↖"),("Tenggara","↖"),("Selatan-Tenggara","↖"),
        ("Selatan","↑"),("Selatan-Barat Daya","↗"),("Barat Daya","↗"),("Barat-Barat Daya","↗"),
        ("Barat","→"),("Barat-Barat Laut","↘"),("Barat Laut","↘"),("Utara-Barat Laut","↘")]
...
# =========================================
# FSI GAUGE — visualisasi bar + legenda
# =========================================
def render_fsi_gauge(fsi_val: float, label: str = "Fisheries Index") -> str:
    """Kembalikan HTML gauge FSI dengan warna sesuai klasifikasi."""
    status = get_fsi_status(fsi_val)
    pct    = int(np.clip(fsi_val, 0, 100))

    legend_chips = ""
    for lo, hi, text, icon, color, bg, border in FSI_KLASIFIKASI:
        active = "font-weight:700;" if status["text"] == text else "opacity:0.55;"
        legend_chips += (
            f'<span class="fsi-legend-chip" style="background:{bg};border-color:{border};'
            f'color:{color};{active}">{icon} {lo}–{hi}</span>'
        )

    return f"""
<div style="background:#FFFFFF;border:1px solid #D6E4F0;border-radius:8px;padding:16px 20px;">
  <div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:#5A7FA0;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:6px;">{label}</div>
  <div style="display:flex;align-items:baseline;gap:8px;margin-bottom:6px;">
    <span style="font-size:28px;font-weight:800;color:{status['color']};">{fsi_val:.1f}</span>
    <span style="font-size:13px;color:#5A7FA0;">/100</span>
    <span style="font-size:14px;font-weight:700;color:{status['color']};margin-left:4px;">{status['icon']} {status['text']}</span>
  </div>
  <div class="fsi-bar-wrap">
    <div style="height:100%;width:{pct}%;background:{status['color']};border-radius:6px;transition:width 0.5s;"></div>
  </div>
  <div class="fsi-legend-row">{legend_chips}</div>
</div>
"""

def render_ohi_gauge(ohi_val: float) -> str:
    status = get_ohi_status(ohi_val)
    pct    = int(np.clip(ohi_val, 0, 100))
    legend_chips = ""
    ...
    return f"""
<div style="background:#FFFFFF;border:1px solid #D6E4F0;border-radius:8px;padding:16px 20px;">
  <div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:#5A7FA0;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:6px;">Ocean Health Index</div>
  <div style="display:flex;align-items:baseline;gap:8px;margin-bottom:6px;">
    <span style="font-size:28px;font-weight:800;color:{status['color']};">{ohi_val:.1f}</span>
    <span style="font-size:13px;color:#5A7FA0;">/100</span>
    <span style="font-size:14px;font-weight:700;color:{status['color']};margin-left:4px;">{status['icon']} {status['text']}</span>
  </div>
  <div class="fsi-bar-wrap">
    <div style="height:100%;width:{pct}%;background:{status['color']};border-radius:6px;"></div>
  </div>
  <div class="fsi-legend-row">{legend_chips}</div>
</div>
"""
...
def make_wind_rose(df_src, title="Rose Diagram Angin"):
    ...
    fig.update_layout(
        ...
        polar=dict(radialaxis=dict(visible=True, tickfont=dict(size=9, color="#5A7FA0"), gridcolor="#D6E4F0"),
            angularaxis=dict(direction="clockwise", rotation=90, tickmode="array",
                tickvals=[0,45,90,135,180,225,270,315], ticktext=["U","TL","T","TG","S","BD","B","BL"],
        ...
        paper_bgcolor="#FFFFFF", height=380, margin=dict(l=20,r=20,t=40,b=20))
    return fig
...
# =========================================
DO_PH_INFO = """
<div class="data-note-derive">
  🧮 <b>DO & pH tidak tersedia dari API publik secara real-time.</b><br>
  Nilai diderivasi dari persamaan empiris oseanografi:<br>
  &nbsp;· <b>DO</b>: Saturasi oksigen Weiss (1970) Deep-Sea Res. 17:721–735, dikoreksi produktivitas klorofil-a<br>
  &nbsp;· <b>pH</b>: Takahashi et al. (2002) Deep-Sea Res. II 49:1601–1622 (pCO₂–SST) + Lee et al. (2006) GBC 20:GB3023 (alkalinitas–salinitas)<br>
  Input: SST (CMEMS), klorofil-a (CMEMS/MODIS), salinitas (CMEMS), bulan saat ini
</div>
"""
...
<div style="display:flex;justify-content:center;gap:12px;flex-wrap:wrap;margin-bottom:32px;">
  <span style="background:#EBF3FB;border:1px solid #C0D8EE;color:#1E6BB8;font-family:'JetBrains Mono',monospace;font-size:10px;padding:5px 12px;border-radius:4px;">🌊 CMEMS · Arus/SST/Salinitas/Klorofil-a</span>
  <span style="background:#EBF3FB;border:1px solid #C0D8EE;color:#1E6BB8;font-family:'JetBrains Mono',monospace;font-size:10px;padding:5px 12px;border-radius:4px;">🛰 MODIS/ERDDAP · Klorofil-a</span>
  <span style="background:#EBF3FB;border:1px solid #C0D8EE;color:#1E6BB8;font-family:'JetBrains Mono',monospace;font-size:10px;padding:5px 12px;border-radius:4px;">💨 ERA5/Open-Meteo · Angin</span>
  <span style="background:#EBF3FB;border:1px solid #C0D8EE;color:#1E6BB8;font-family:'JetBrains Mono',monospace;font-size:10px;padding:5px 12px;border-radius:4px;">🌧 BMKG/Open-Meteo · Gelombang</span>
</div>
""", unsafe_allow_html=True)
...
<div style="background:#FFFFFF;border:1px solid #D6E4F0;border-top:3px solid #1E6BB8;border-radius:8px;padding:32px 28px;text-align:center;margin-bottom:12px;">
  ...
  <p style="color:#5A7FA0;font-size:14px;margin:0 0 16px;line-height:1.6;">Peta zona tangkap, kondisi gelombang, dan rekomendasi area melaut hari ini.</p>
</div>""", unsafe_allow_html=True)
        if st.button("Masuk sebagai Nelayan →", use_container_width=True, key="btn_nelayan"):
            st.session_state.role = "nelayan"; st.session_state.page = "dashboard"; st.rerun()
    with c2:
        st.markdown("""
...
# =========================================
with st.sidebar:
    st.markdown("""<div style="padding:20px 0 12px;text-align:center;">
  ...
  <div style="font-size:11px;color:#7BAFD4;margin-top:3px;">Ocean Intelligence Platform</div>
</div>""", unsafe_allow_html=True)
    st.markdown("---")
    ...
    if st.session_state.prev_mode != mode:
        for k in [k for k in st.session_state.keys() if k.startswith("prophet_")]:
    ...
    if mode == "Historis":
        ...
        musim_map_dict = {"Musim Barat":[12,1,2],"Peralihan I":[3,4,5],
        ...
        st.markdown("""<div class="data-note">🛰 Sumber data aktif:<br>
· CMEMS → Arus, SST, Salinitas, Klorofil-a<br>
· MODIS/ERDDAP → Klorofil-a (publik)<br>
· ERA5/Open-Meteo → Angin<br>
· BMKG/Open-Meteo → Gelombang<br><br>
🧮 DO &amp; pH → diderivasi dari jurnal<br>(Weiss 1970, Takahashi et al. 2002)</div>""", unsafe_allow_html=True)
        ...
        if st.button("🔄 Muat Data Real-Time Sekarang", use_container_width=True, key="btn_rt"):
            with st.spinner("Menghubungi semua API..."):
                ...
                st.session_state.rt_data     = result.get("data")
                st.session_state.api_status  = result.get("status", {})
                st.session_state.api_errors  = result.get("errors", {})
        ...
        if st.session_state.api_status:
            ...
            for api_name, ok in st.session_state.api_status.items():
                ...
                err   = st.session_state.api_errors.get(api_name, "")
                hint  = f" · <i>{err[:40]}</i>" if (not ok and err) else ""
                st.markdown(f"<span style='font-size:11px;font-family:monospace'>{icon} {api_name} — {label}{hint}</span>", unsafe_allow_html=True)
            st.markdown("<span style='font-size:11px;font-family:monospace'>🧮 DO/pH — Derivasi jurnal (selalu)</span>", unsafe_allow_html=True)
        ...
        st.markdown("""<div class="data-note">🤖 Prediksi Prophet dilatih pada data historis 2001–2020.</div>""", unsafe_allow_html=True)
    ...
    st.markdown("""<div style="background:#162A42;border-radius:6px;padding:12px 14px;margin-top:12px;">
...
if mode == "Historis":
    ...
    if st.session_state.role == "nelayan":
        ...
        fsi_status = get_fsi_status(mean_fsi)

        st.markdown(f"""<div class="page-header">
  <div class="eyebrow">🎣 Nelayan · <span class="mode-badge-hist">📂 HISTORIS</span> &nbsp;{waktu_label}</div>
  <h1>Peta Zona Tangkap — Laut Arafura</h1>
</div>""", unsafe_allow_html=True)

        col_s1, col_s2, col_s3, col_s4 = st.columns([2,1,1,1])
        with col_s1:
            st.markdown(render_fsi_gauge(mean_fsi, "Status Zona Melaut"), unsafe_allow_html=True)
        ...
        with tabs_n[2]:
            ...
            with col_r1:
                ...
                if fsi_status["text"] in ("SANGAT BAIK","BAIK"):
                    st.success(f"**Area oranye/merah direkomendasikan.** Nutrisi laut melimpah. Arus menuju **{arah_arus}** {ikon_arus}, angin dari **{arah_angin}** {ikon_angin}.")
                elif fsi_status["text"] == "NORMAL":
                    st.info(f"**Kondisi normal.** Ikuti arus ke **{arah_arus}** {ikon_arus}. Angin dari **{arah_angin}** {ikon_angin}.")
                elif fsi_status["text"] == "WASPADA":
                    st.warning(f"**Potensi tangkapan rendah.** Disarankan ke pesisir teluk & muara. Arus ke **{arah_arus}** {ikon_arus}.")
                else:
                    st.error(f"**⚠ Kondisi kritis.** Tidak disarankan melaut jauh. Arus ke **{arah_arus}** {ikon_arus}.")
            with col_r2:
                st.markdown('<div class="section-label">KONDISI PERAIRAN</div>', unsafe_allow_html=True)
                st.markdown(f"""<div style="background:#FFFFFF;border:1px solid #D6E4F0;border-radius:8px;padding:20px;">
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
    <div style="text-align:center;"><div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:#5A7FA0;text-transform:uppercase;margin-bottom:4px;">Klorofil-a</div>
      <div style="font-size:22px;font-weight:700;color:#0D1F33;">{df_map['chla'].mean():.3f} <span style="font-size:12px;color:#5A7FA0;">mg/m³</span></div></div>
    <div style="text-align:center;"><div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:#5A7FA0;text-transform:uppercase;margin-bottom:4px;">Dissolved O₂</div>
      <div style="font-size:22px;font-weight:700;color:#0D1F33;">{df_map['do'].mean():.2f} <span style="font-size:12px;color:#5A7FA0;">mg/L</span></div></div>
    <div style="text-align:center;"><div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:#5A7FA0;text-transform:uppercase;margin-bottom:4px;">Arah Arus</div>
      <div style="font-size:18px;font-weight:700;color:#0D1F33;">{ikon_arus} {arah_arus}</div></div>
    <div style="text-align:center;"><div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:#5A7FA0;text-transform:uppercase;margin-bottom:4px;">Angin Dari</div>
      <div style="font-size:18px;font-weight:700;color:#0D1F33;">{ikon_angin} {arah_angin}</div></div>
  </div></div>""", unsafe_allow_html=True)

    # ── AKADEMISI ────────────────────────────────────────────
    else:
        st.markdown(f"""<div class="page-header">
  <div class="eyebrow">🔬 Akademisi · <span class="mode-badge-hist">📂 HISTORIS 2001–2020</span> · {waktu_label}</div>
  <h1>Analisis Parameter — {PARAM_LABELS.get(parameter, parameter)}</h1>
        ...
        # Gauge FSI/OHI di Akademisi
        if parameter in ("Fisheries_Index","Ocean_Health_Index"):
            mean_val = float(df_map[parameter].mean())
            if parameter == "Fisheries_Index":
                st.markdown(render_fsi_gauge(mean_val), unsafe_allow_html=True)
                st.markdown("""<div class="data-note">📖 Klasifikasi FSI berdasarkan Zainuddin et al. (2006, 2017) — studi zona potensial perikanan Arafura-Banda. Normalisasi menggunakan rentang tetap berdasarkan literatur oseanografi.</div>""", unsafe_allow_html=True)
            else:
                st.markdown(render_ohi_gauge(mean_val), unsafe_allow_html=True)

        if parameter in ("do","ph"):
            st.markdown(DO_PH_INFO, unsafe_allow_html=True)
        ...
        with tabs[0]:
            cmap_dict = {'Fisheries_Index':'Turbo','chla':'Turbo','Ocean_Health_Index':'Blues',
                'do':'Blues','ph':'Viridis','salinitas':'YlOrBr','sst':'Plasma','ssta':'RdBu','current_speed':'Teal'}
            ...
            st.markdown(f"""<span class="coord-tag">4°S – 12°S</span><span class="coord-tag">129°E – 144°E</span><span class="coord-tag">Grid 100×80</span>""", unsafe_allow_html=True)
        ...
        with tabs[1]:
            ...
            if len(y_vals) >= 2:
                ...
                y_trend = y_vals.copy(); slope_label = "Tren"; z = [0,0]
            y_lo = float(min(y_vals.min(), y_trend.min()))
            y_hi = float(max(y_vals.max(), y_trend.max()))
            span = y_hi - y_lo
            pad  = span * 0.10 if span > 0 else 0.01
            fig_ts = go.Figure()
            ...
            if len(y_vals) >= 2:
                fig_ts.add_trace(go.Scatter(x=df_ts["time"], y=y_trend, mode="lines",
                    name=slope_label, line=dict(color="#D4811A", width=2, dash="dot")))
            if not df_hist.empty:
                df_highlight = df_ts[df_ts["time"].isin(df_hist["time"])]
                if not df_highlight.empty:
                    fig_ts.add_trace(go.Scatter(x=df_highlight["time"],
                        y=df_highlight[parameter].to_numpy(dtype=float),
                        mode="markers", name=f"Periode: {waktu_label}",
                        marker=dict(color="#E85A0C", size=8, line=dict(color="#FFFFFF", width=1.5))))
            layout_base = {k: v for k, v in PLOTLY_LAYOUT.items() if k != "xaxis"}
            fig_ts.update_layout(**layout_base,
                ...
                legend=dict(font=dict(color="#3A5070", size=11),
                    bgcolor="rgba(255,255,255,0.9)", bordercolor="#D6E4F0", borderwidth=1),
                height=420, hovermode="x unified")
            fig_ts.update_yaxes(range=[y_lo - pad, y_hi + pad], autorange=False)
            st.plotly_chart(fig_ts, use_container_width=True)
            st.caption(f"Data historis CSV 2001–2020 · {len(df_ts)} titik"
        ...
        with tabs[2]:
            ...
            for i, (label, key, icon) in enumerate([
                ...
                sc[i].markdown(f"""<div style="background:#FFFFFF;border:1px solid #D6E4F0;border-radius:8px;padding:18px;text-align:center;">
  ...
  <div style="font-size:18px;font-weight:700;color:#0D1F33;margin-top:4px;">{val}</div></div>""", unsafe_allow_html=True)
            st.markdown("<div style='height:12px'></div>", unsafe_allow_html=True)
            sc2 = st.columns(3)
            for i, (label, key) in enumerate([("25th Percentile","25%"),("Median (50%)","50%"),("75th Percentile","75%")]):
                sc2[i].markdown(f"""<div style="background:#FFFFFF;border:1px solid #D6E4F0;border-radius:8px;padding:18px;text-align:center;">
  ...
  <div style="font-size:18px;font-weight:700;color:#0D1F33;">{desc.loc[key,parameter]:.4f}</div></div>""", unsafe_allow_html=True)
            if parameter in ("Fisheries_Index","Ocean_Health_Index"):
                st.markdown("<div style='height:12px'></div>", unsafe_allow_html=True)
                st.markdown('<div class="section-label">KLASIFIKASI NILAI</div>', unsafe_allow_html=True)
                klasifikasi = FSI_KLASIFIKASI if parameter == "Fisheries_Index" else OHI_KLASIFIKASI
                cols_k = st.columns(len(klasifikasi))
                for i, (lo, hi, text, icon, color, bg, border) in enumerate(klasifikasi):
                    with cols_k[i]:
                        st.markdown(f"""<div style="background:{bg};border:1px solid {border};border-radius:6px;padding:10px;text-align:center;">
  <div style="font-size:16px;">{icon}</div>
  <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:{color};font-weight:700;margin-top:3px;">{text}</div>
  <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:#5A7FA0;">{lo}–{hi}</div></div>""", unsafe_allow_html=True)
        ...
        with tabs[3]:
            ...
            SHORT_CORR = {"uo":"UO","vo":"VO","sst":"SST","ssta":"SSTA","ph":"pH","do":"DO",
            ...
            labels = list(corr.columns); n = len(labels); vals = corr.values
            zmin, zmax = float(np.nanmin(vals)), float(np.nanmax(vals)); rng_z = (zmax - zmin) or 1.0
            fig_corr = go.Figure(go.Heatmap(z=vals, x=labels, y=labels,
        ...
        if tampilkan_rose:
            with tabs[4]:
                df_rose_ak = df_hist if not df_hist.empty else df
                if parameter in ("angin_u","angin_v"):
                    st.plotly_chart(make_wind_rose(df_rose_ak, f"Angin · {waktu_label}"), use_container_width=True)
                else:
                    st.plotly_chart(make_wave_rose(df_rose_ak, f"Gelombang · {waktu_label}"), use_container_width=True)
...
# =========================================
    ...
    if st.session_state.last_update is None:
        st.markdown("""<div class="page-header"><div class="eyebrow">🛰 Mode Real-Time</div><h1>Data Oseanografi Terkini</h1></div>""", unsafe_allow_html=True)
        st.markdown("""<div class="rt-empty-box">
  <div style="font-size:48px;margin-bottom:16px;">🛰</div>
  <div style="font-size:18px;font-weight:700;color:#0D1F33;margin-bottom:8px;">Belum Ada Data Real-Time</div>
  <div style="color:#5A7FA0;font-size:14px;line-height:1.6;">Klik <b>🔄 Muat Data Real-Time Sekarang</b> di sidebar kiri.</div>
</div>""", unsafe_allow_html=True)
        st.stop()
    ...
    if df_rt_raw is not None and not df_rt_raw.empty:
        ...
        n_ok  = sum(v for v in active_apis.values())
        n_all = len(active_apis)
        sumber_badge = f"🟢 {n_ok}/{n_all} API aktif"
    else:
        df_rt   = df[df["month"] == now_utc.month].copy()
        is_live = False
        n_ok, n_all = 0, 4
        sumber_badge = "🟡 Estimasi klimatologis"

    # Pastikan kolom indeks ada dan benar
    if "current_speed" not in df_rt.columns:
        df_rt["current_speed"] = np.sqrt(df_rt["uo"]**2 + df_rt["vo"]**2)

    # Hitung ulang OHI/FSI dengan normalisasi fixed-range
    ohi_vals, fsi_vals = [], []
    for _, row in df_rt.iterrows():
        o, f = hitung_indeks(row["chla"], row["do"], row["ph"],
                             row["salinitas"], row["gelombang"], row["current_speed"])
        ohi_vals.append(o); fsi_vals.append(f)
    df_rt["Ocean_Health_Index"] = ohi_vals
    df_rt["Fisheries_Index"]    = fsi_vals
    ...
    def _rt_api_banner():
        if is_live:
            ...
            for api_name, ok in st.session_state.api_status.items():
                icon_ = "✓" if ok else "✗"
                parts.append(f"{icon_} {api_name}")
            detail = " &nbsp;|&nbsp; ".join(parts)
            st.markdown(f"""<div style="background:#EDFAF3;border:1px solid #9FD9BE;border-left:4px solid #00895A;border-radius:6px;padding:10px 16px;margin-bottom:16px;font-family:'JetBrains Mono',monospace;font-size:11px;color:#00895A;">
  🟢 <b>{n_ok}/{n_all} API aktif</b> · {update_wib}<br>
  <span style="font-size:10px;color:#2A7050;">{detail}</span><br>
  <span style="font-size:10px;color:#5A3080;">🧮 DO &amp; pH — Derivasi jurnal (Weiss 1970 · Takahashi et al. 2002)</span>
</div>""", unsafe_allow_html=True)
        else:
            st.markdown(f"""<div style="background:#FEF6E8;border:1px solid #F0C070;border-left:4px solid #D4811A;border-radius:6px;padding:10px 16px;margin-bottom:16px;font-family:'JetBrains Mono',monospace;font-size:11px;color:#8a5a00;">
  🟡 <b>Semua API tidak merespons</b> — estimasi klimatologis bulan {now_utc.strftime('%B')}. Tekan 🔄 untuk coba lagi.<br>
  <span style="font-size:10px;color:#5A3080;">🧮 DO &amp; pH — Derivasi jurnal (Weiss 1970 · Takahashi et al. 2002)</span>
</div>""", unsafe_allow_html=True)
    ...
    if st.session_state.role == "nelayan":
        ...
        st.markdown(f"""<div class="page-header">
  <div class="eyebrow">🎣 Nelayan · <span class="update-badge"><span class="pulse"></span> REAL-TIME</span> &nbsp; {update_wib}</div>
  <h1>Kondisi Laut Sekarang — Laut Arafura</h1>
</div>""", unsafe_allow_html=True)
        _rt_api_banner()

        col_s1, col_s2, col_s3, col_s4 = st.columns([2,1,1,1])
        with col_s1: st.markdown(render_fsi_gauge(mean_fsi, "Status Zona Melaut Hari Ini"), unsafe_allow_html=True)
        with col_s2: st.metric("Potensi Ikan (FSI)", f"{mean_fsi:.0f}", "/ 100")
        ...
        tabs_rt = st.tabs(["  Peta  ", "  Detail Parameter  ", "  Rose Diagram  ", "  Rekomendasi  "])
        ...
        with tabs_rt[1]:
            ...
            modis_ok = st.session_state.api_status.get("NASA MODIS", False)

            def _src(ok, live_name, fallback="Klimatologis"):
                return (live_name if ok else fallback), ("🟢" if ok else "🟡")

            param_rt_list = [
                ("🌡 SST",         f"{float(df_rt['sst'].mean()):.2f} °C",    *_src(cmems_ok, "CMEMS")),
                ("🧪 Salinitas",   f"{float(df_rt['salinitas'].mean()):.2f} PSU", *_src(cmems_ok, "CMEMS")),
                ("🌿 Klorofil-a",  f"{float(df_rt['chla'].mean()):.3f} mg/m³", *_src(modis_ok or cmems_ok, "CMEMS/MODIS")),
                ("💧 Dissolved O₂",f"{float(df_rt['do'].mean()):.2f} mg/L",  "Derivasi Weiss (1970)", "🧮"),
                ("🧪 pH",          f"{float(df_rt['ph'].mean()):.3f}",        "Derivasi Takahashi (2002)", "🧮"),
                ("🌊 Gelombang",   f"{float(df_rt['gelombang'].mean()):.2f} m", *_src(bmkg_ok, "BMKG/Open-Meteo")),
                ("💨 Angin U",     f"{float(df_rt['angin_u'].mean()):.2f} m/s", *_src(era5_ok, "ERA5/Open-Meteo")),
                ("💨 Angin V",     f"{float(df_rt['angin_v'].mean()):.2f} m/s", *_src(era5_ok, "ERA5/Open-Meteo")),
            ]
            cols_rt = st.columns(4)
            for idx, (label, val, src, ic) in enumerate(param_rt_list):
                purple_ic = ic == "🧮"
                color_src = "#6D28D9" if purple_ic else ("#00895A" if ic == "🟢" else "#D4811A")
                with cols_rt[idx % 4]:
                    st.markdown(f"""<div style="background:#FFFFFF;border:1px solid {'#E8E0F5' if purple_ic else '#D6E4F0'};border-radius:8px;padding:14px;margin-bottom:10px;">
  <div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:#5A7FA0;text-transform:uppercase;">{label}</div>
  <div style="font-size:18px;font-weight:700;color:#0D1F33;margin:4px 0;">{val}</div>
  <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:{color_src};">{ic} {src}</div>
</div>""", unsafe_allow_html=True)
            st.markdown(DO_PH_INFO, unsafe_allow_html=True)

        with tabs_rt[2]:
            rc1, rc2 = st.columns(2)
            with rc1: st.plotly_chart(make_wind_rose(df_rt, f"Angin · {update_wib}"), use_container_width=True)
            with rc2: st.plotly_chart(make_wave_rose(df_rt, f"Gelombang · {update_wib}"), use_container_width=True)
        ...
        with tabs_rt[3]:
            ...
            with col_r1:
                st.markdown('<div class="section-label">REKOMENDASI ZONA</div>', unsafe_allow_html=True)
                if fsi_status["text"] in ("SANGAT BAIK","BAIK"):
                    st.success(f"**Area oranye/merah direkomendasikan.** Nutrisi melimpah — turunkan jaring di perairan dalam. Arus ke **{arah_arus_rt}** {ikon_arus_rt}, angin dari **{arah_angin_rt}** {ikon_angin_rt}.")
                elif fsi_status["text"] == "NORMAL":
                    st.info(f"**Kondisi normal.** Ikuti arus ke **{arah_arus_rt}** {ikon_arus_rt}. Angin dari **{arah_angin_rt}** {ikon_angin_rt}.")
                elif fsi_status["text"] == "WASPADA":
                    st.warning(f"**Potensi tangkapan rendah.** Disarankan ke pesisir teluk & muara. Waspadai arus ke **{arah_arus_rt}** {ikon_arus_rt}.")
                else:
                    st.error(f"**⚠ Kondisi kritis.** Tidak disarankan melaut jauh.")
            with col_r2:
    ...
    # ── AKADEMISI · RT ───────────────────────────────────────
        ...
        st.markdown(f"""<div class="page-header">
  <div class="eyebrow">🔬 Akademisi · <span class="update-badge"><span class="pulse"></span> REAL-TIME</span> &nbsp; {update_wib}</div>
  <h1>Data Terkini — {PARAM_LABELS.get(parameter, parameter)}</h1>
        ...
        # Gauge untuk FSI/OHI di RT Akademisi
        if parameter in ("Fisheries_Index","Ocean_Health_Index"):
            mean_val = float(df_map_rt[parameter].mean())
            if parameter == "Fisheries_Index":
                st.markdown(render_fsi_gauge(mean_val), unsafe_allow_html=True)
            else:
                st.markdown(render_ohi_gauge(mean_val), unsafe_allow_html=True)
        ...
        with tabs_rt_ak[1]:
            ...
            fig_compare.add_trace(go.Scatter(x=df_ts_rt["time"], y=df_ts_rt[parameter].to_numpy(dtype=float),
                mode="lines", name="Historis 2001–2020", line=dict(color="#A8C8E8", width=1.5)))
            fig_compare.add_hline(y=rt_val, line_color="#E85A0C", line_width=2.5, line_dash="dash",
                annotation_text=f"Nilai RT: {rt_val:.3f}", annotation_font=dict(color="#E85A0C", size=11))
            ...
            if parameter in ("Fisheries_Index","Ocean_Health_Index"):
                ...
                for i, (lo, hi, text, icon, color, bg, border) in enumerate(klasifikasi):
                    with cols_k[i]:
                        st.markdown(f"""<div style="background:{bg};border:1px solid {border};border-radius:6px;padding:10px;text-align:center;">
  <div style="font-size:16px;">{icon}</div>
  <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:{color};font-weight:700;">{text}</div>
  <div style="font-family:'JetBrains Mono',monospace;font-size:9px;color:#5A7FA0;">{lo}–{hi}</div></div>""", unsafe_allow_html=True)
...
# =========================================
  ...
  <h1>Proyeksi Kondisi Laut — {bulan_pred}</h1>
        ...
        if all(f"prophet_nelayan_{kp}" in st.session_state for kp in KEY_PARAMS_NELAYAN):
            ...
            # FSI prediksi untuk bulan target
            ...
            st.markdown(render_fsi_gauge(fsi_pred, f"Proyeksi FSI — {bulan_pred}"), unsafe_allow_html=True)
            ...
            fig_fsi_pred.update_layout(**PLOTLY_LAYOUT, title=f"Prediksi FSI · {bulan_pred}", height=380)
            ...
            fsi_status_pred = get_fsi_status(fsi_pred)

            st.markdown('<div class="section-label">PROYEKSI REKOMENDASI</div>', unsafe_allow_html=True)
            if fsi_status_pred["text"] in ("SANGAT BAIK","BAIK"):
                st.success(f"**Proyeksi {bulan_pred}: {fsi_status_pred['text']} (FSI {fsi_pred:.1f}/100).** Kondisi laut diprediksi mendukung penangkapan optimal. Arus menuju **{arah_arus_pred}** {ikon_arus_pred}, angin dari **{arah_angin_pred}** {ikon_angin_pred}.")
            elif fsi_status_pred["text"] == "NORMAL":
                st.info(f"**Proyeksi {bulan_pred}: NORMAL (FSI {fsi_pred:.1f}/100).** Tangkapan rata-rata — ikuti arus ke **{arah_arus_pred}** {ikon_arus_pred}.")
            elif fsi_status_pred["text"] == "WASPADA":
                st.warning(f"**Proyeksi {bulan_pred}: WASPADA (FSI {fsi_pred:.1f}/100).** Pertimbangkan mengurangi aktivitas melaut jauh.")
            else:
                st.error(f"**Proyeksi {bulan_pred}: KRITIS (FSI {fsi_pred:.1f}/100).** Tidak disarankan melaut jauh.")
        else:
            st.info("Klik **▶ Jalankan Prediksi** untuk melihat proyeksi.")
  ...
  <h1>Model Prediksi — {PARAM_LABELS.get(parameter, parameter)}</h1>
        ...
        st.markdown("""<div class="data-note">Model Prophet dilatih pada data historis 2001–2020. Prediksi mencakup tren, seasonalitas, dan interval kepercayaan 80%.</div>""", unsafe_allow_html=True)
        if parameter in ("do","ph"):
            st.markdown(DO_PH_INFO, unsafe_allow_html=True)
        ...
        if run_prophet:
            with st.spinner(f"Melatih Prophet · {PARAM_LABELS.get(parameter, parameter)}..."):
        ...
        if prophet_key in st.session_state:
            ...
            with tabs_pred[0]:
                ...
                fig_fc.add_trace(go.Scatter(x=df_ts2["time"], y=df_ts2[parameter].to_numpy(dtype=float),
                    mode="lines", name="Historis (2001–2020)", line=dict(color="#1E6BB8", width=1.5)))
                ...
                if not target_rows.empty:
                    fig_fc.add_trace(go.Scatter(x=target_rows["ds"], y=target_rows["yhat"],
                ...
                fig_fc.update_layout(**PLOTLY_LAYOUT,
                    ...
                    legend=dict(font=dict(color="#3A5070",size=11), bgcolor="rgba(255,255,255,0.9)", bordercolor="#D6E4F0"),
                    height=430)
                with col_ph1:
                ...
                if not target_rows.empty:
                    ...
                    st.markdown(f"""<div style="background:#FEF6E8;border:1px solid #F0C070;border-left:4px solid #D4811A;border-radius:6px;padding:12px 16px;font-family:'JetBrains Mono',monospace;font-size:12px;color:#8a5a00;">
  ⭐ Nilai Prediksi <b>{bulan_pred}</b>: <b>{tv:.4f}</b> &nbsp;|&nbsp; Interval 80%: [{tl:.4f} — {tu:.4f}]</div>""", unsafe_allow_html=True)

                if parameter == "Fisheries_Index":
                    st.markdown(render_fsi_gauge(tv if not target_rows.empty else 50.0, f"Proyeksi FSI — {bulan_pred}"), unsafe_allow_html=True)
            ...
            with tabs_pred[1]:
                ...
                if comp_cols:
                    ...
                    for i, comp in enumerate(comp_cols):
                        fig_comp.add_trace(go.Scatter(x=forecast_df["ds"], y=forecast_df[comp],
                            mode="lines", name=comp.capitalize(),
                            line=dict(color=["#1E6BB8","#E85A0C","#00895A"][i % 3], width=1.8)))
