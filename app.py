...
============================================================
Real-Time  : CMEMS · ERA5/Open-Meteo · BMKG
Historis   : rangkuman_historis_20tahun.csv
...
PLOTLY_LAYOUT = dict(
    ...
    font=dict(family="Plus Jakarta Sans", color="#3A6880", size=12),
    xaxis=dict(gridcolor="#C8E8F8", zerolinecolor="#C8E8F8", color="#5A88A8"),
    yaxis=dict(gridcolor="#C8E8F8", zerolinecolor="#C8E8F8", color="#5A88A8"),
    title_font=dict(color="#003F5E", size=15, family="Plus Jakarta Sans"),
)
...
df["Ocean_Health_Index"] = (
    0.30 * normalisasi_global(df["do"], 4.5, 7.5) +
    0.25 * normalisasi_global(df["ph"], 7.9, 8.4) +
    0.20 * suitabilitas_optimal(df["chla"], 0.05, 0.10, 0.40, 0.80) +
    0.15 * suitabilitas_optimal(df["salinitas"], 32.0, 33.5, 35.0, 36.5) +
    0.10 * suitabilitas_optimal(df["sst"], 22.0, 26.0, 30.0, 32.0)
) * 100
...
df["Fisheries_Index"] = (
    ...
    0.10 * (1 - normalisasi_global(df["gelombang"], 0.2, 2.5))
) * 100
...
def render_map(df_map, z_col, colorscale, height=520):
    ...
    if geojson_prov:
        map_layers.append(dict(
            ...
            type="line", color="#0077B6", opacity=0.4, line=dict(width=1.0),
        ))
    fig.update_layout(
        ...
        coloraxis_colorbar=dict(
            ...
            bgcolor="rgba(255,255,255,0.95)", bordercolor="#C8E8F8",
            tickfont=dict(color="#3A6880", size=10, family="JetBrains Mono"),
            title=dict(font=dict(color="#3A6880", size=10, family="JetBrains Mono")),
        )
    )
    return fig
...
POLAR_LAYOUT_BASE = dict(
    ...
    legend=dict(
        font=dict(size=10, color="#3A6880", family="Plus Jakarta Sans"),
        bgcolor="rgba(255,255,255,0.9)", bordercolor="#C8E8F8",
    ),
)
...
def make_wind_rose(df_src, title="Rose Diagram Angin"):
    ...
    for k, (s0, s1) in enumerate(zip(speed_bins[:-1], speed_bins[1:])):
        ...
        fig.add_trace(go.Barpolar(r=counts, theta=bin_edges[:-1], width=[360/n_bins]*n_bins,
            ...
            marker_line_color="white", marker_line_width=0.8, opacity=0.92))
    fig.update_layout(
        title=dict(text=title, font=dict(color="#003F5E", size=13, family="Plus Jakarta Sans"), x=0.5),
        polar=dict(
            radialaxis=dict(visible=True, tickfont=dict(size=9, color="#5A88A8"), gridcolor="#C8E8F8"),
            angularaxis=dict(direction="clockwise", rotation=90,
                tickmode="array", tickvals=[0,45,90,135,180,225,270,315],
                ticktext=["U","TL","T","TG","S","BD","B","BL"],
                tickfont=dict(size=10, color="#003F5E", family="Plus Jakarta Sans"), gridcolor="#C8E8F8"),
        ),
        **POLAR_LAYOUT_BASE,
    )
    return fig
...
<div style="background:#FFFFFF;border:1.5px solid #C8E8F8;border-radius:16px;padding:32px 28px;text-align:center;margin-bottom:14px;box-shadow:0 4px 16px rgba(0,119,182,0.10);">
  ...
  <div style="display:flex;justify-content:center;gap:6px;flex-wrap:wrap;">
    ...
    <span style="background:rgba(0,201,224,0.12);color:#0077B6;font-size:10px;padding:4px 10px;border-radius:20px;font-family:'JetBrains Mono',monospace;border:1px solid rgba(0,201,224,0.3);font-weight:600;">Gelombang Live</span>
    <span style="background:rgba(0,201,224,0.12);color:#0077B6;font-size:10px;padding:4px 10px;border-radius:20px;font-family:'JetBrains Mono',monospace;border:1px solid rgba(0,201,224,0.3);font-weight:600;">Arah Arus</span>
  </div>
</div>
""", unsafe_allow_html=True)
...
<div style="background:#FFFFFF;border:1.5px solid #C8E8F8;border-radius:16px;padding:32px 28px;text-align:center;margin-bottom:14px;box-shadow:0 4px 16px rgba(0,119,182,0.10);">
  ...
  <div style="display:flex;justify-content:center;gap:6px;flex-wrap:wrap;">
    ...
    <span style="background:rgba(0,201,167,0.12);color:#00875A;font-size:10px;padding:4px 10px;border-radius:20px;font-family:'JetBrains Mono',monospace;border:1px solid rgba(0,201,167,0.3);font-weight:600;">Korelasi</span>
    <span style="background:rgba(0,201,167,0.12);color:#00875A;font-size:10px;padding:4px 10px;border-radius:20px;font-family:'JetBrains Mono',monospace;border:1px solid rgba(0,201,167,0.3);font-weight:600;">Time Series</span>
  </div>
</div>
""", unsafe_allow_html=True)
...
<div style="text-align:center;margin-top:20px;">
  <div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:#90CBE8;letter-spacing:0.12em;">
    SUMBER DATA: CMEMS · ERA5 · BMKG · 2001–2020
  </div>
</div>
""", unsafe_allow_html=True)
    st.stop()
...
def build_map_from_df(df_src):
    ...
    return build_spatial_grid(
        uo=_mean_or(df_src,"uo",-0.05), vo=_mean_or(df_src,"vo",-0.01),
        sst=_mean_or(df_src,"sst",28.5), do=_mean_or(df_src,"do",6.2),
        ph=_mean_or(df_src,"ph",8.12), chla=_mean_or(df_src,"chla",0.22),
        sal=_mean_or(df_src,"salinitas",34.2), wave=_mean_or(df_src,"gelombang",0.8),
        angin_u=_mean_or(df_src,"angin_u",-1.5), angin_v=_mean_or(df_src,"angin_v",-0.5),
        ssta=_mean_or(df_src,"ssta",0.0),
        month_seed=active_month, year_seed=active_year,
    )
...
""", unsafe_allow_html=True)
        ...
        with col_s1:
            st.markdown(render_status_card(status, mean_fsi, waktu_label), unsafe_allow_html=True)
        ...
        with tabs[3]:
            ...
            fig_corr = go.Figure(go.Heatmap(
                ...
                colorbar=dict(thickness=12, len=0.7,
                    tickfont=dict(size=10, family="JetBrains Mono", color="#003F5E")),
            ))
            annotations = []
            for i in range(n):
                for j in range(n):
                    ...
                    annotations.append(dict(x=labels[j], y=labels[i], text=f"{v:.2f}", showarrow=False,
                        font=dict(size=10, color="#FFFFFF" if frac > 0.52 else "#003F5E", family="JetBrains Mono")))
            chart_h = max(560, 46*n+170)
            fig_corr.update_layout(
                annotations=annotations,
                title=dict(text="Matriks Korelasi Pearson — Parameter Oseanografi",
                    font=dict(color="#003F5E",size=15,family="Plus Jakarta Sans"), x=0.01),
                paper_bgcolor="#FFFFFF", plot_bgcolor="#FFFFFF", height=chart_h,
                margin=dict(l=10,r=10,t=60,b=10),
                xaxis=dict(tickangle=45, side="bottom", tickfont=dict(size=12,color="#003F5E",family="Plus Jakarta Sans")),
                yaxis=dict(autorange="reversed", tickfont=dict(size=12,color="#003F5E",family="Plus Jakarta Sans")),
            )
        ...
        if tampilkan_rose:
            with tabs[4]:
                ...
                if parameter in ("angin_u","angin_v"):
                    ...
                    st.markdown('<div class="data-note">Rose diagram angin dibangun dari komponen zonal (U) dan meridional (V) — konvensi meteorologis (arah dari mana angin datang).</div>', unsafe_allow_html=True)
                else:
...
""", unsafe_allow_html=True)
        ...
        col_s1, col_s2, col_s3, col_s4 = st.columns([2,1,1,1])
        with col_s1:
            st.markdown(render_status_card(status, mean_fsi, "Hari Ini"), unsafe_allow_html=True)
        with col_s2: st.metric("Potensi Ikan (FSI)", f"{mean_fsi:.0f}", "/ 100")
...
""", unsafe_allow_html=True)
        ...
        with tabs_rt_ak[1]:
            ...
            fig_compare.add_trace(go.Scatter(
                ...
                line=dict(color="#BEE9F5", width=1.8)))
            fig_compare.add_hline(
                y=rt_val, line_color="#FF6B6B", line_width=2.5, line_dash="dash",
                annotation_text=f"Nilai RT: {rt_val:.3f}",
                annotation_font=dict(color="#FF6B6B", size=11))
            fig_compare.update_layout(**PLOTLY_LAYOUT,
                title=f"Nilai Terkini vs Klimatologi · {PARAM_LABELS.get(parameter,parameter)}", height=380)
...
""", unsafe_allow_html=True)
        ...
        if results_ready:
            ...
            for idx, kp in enumerate(KEY_PARAMS_NELAYAN):
                fc, _ = st.session_state[f"Prediksi_nelayan_{kp}"]
                future_only = fc[fc["ds"] > df["time"].max()]
                target_row  = future_only[future_only["ds"].dt.month == month_idx_pred]
                if not target_row.empty:
                    pv = float(target_row["yhat"].iloc[0])
                    lo = float(target_row["yhat_lower"].iloc[0])
                    hi = float(target_row["yhat_upper"].iloc[0])
                    with cols_pred[idx]:
                        st.markdown(f"""
<div class="fun-card" style="text-align:center;">
...
""", unsafe_allow_html=True)
            ...
            fig_fsi_pred.add_trace(go.Scatter(x=df_ts_fsi["time"], y=df_ts_fsi["Fisheries_Index"].to_numpy(dtype=float),
                mode="lines", name="Historis", line=dict(color="#BEE9F5", width=1.5)))
            fig_fsi_pred.add_trace(go.Scatter(x=future_only["ds"], y=future_only["yhat"],
                mode="lines", name="Prediksi", line=dict(color="#FF6B6B", width=2.8)))
        ...
        if Prediksi_key in st.session_state:
            ...
            with tabs_pred[0]:
                ...
                fig_fc.add_trace(go.Scatter(x=df_ts2["time"], y=df_ts2[parameter].to_numpy(dtype=float),
                    mode="lines", name="Data Historis (2001–2020)", line=dict(color="#0077B6", width=1.8)))
                ...
                if not target_rows.empty:
                    fig_fc.add_trace(go.Scatter(x=target_rows["ds"], y=target_rows["yhat"],
                        ...
                        marker=dict(color="#FFD166", size=12, symbol="star",
                                    line=dict(color="#C07200", width=2))))
...
""", unsafe_allow_html=True)
            ...
            with tabs_pred[1]:
                ...
                if comp_cols:
                    fig_comp = go.Figure()
                    colors_comp = ["#0077B6","#FF6B6B","#00C9A7"]
                    for i, comp in enumerate(comp_cols):
                        fig_comp.add_trace(go.Scatter(x=forecast_df["ds"], y=forecast_df[comp],
                            ...
                            line=dict(color=colors_comp[i % len(colors_comp)], width=2.2)))
                    fig_comp.update_layout(**PLOTLY_LAYOUT, height=340,
                        title=f"Dekomposisi Komponen · {PARAM_LABELS.get(parameter,parameter)}",
                        legend=dict(font=dict(color="#3A6880",size=11)))
