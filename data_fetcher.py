...
==============================================================================
Sumber data aktif:
  1. CMEMS  → Arus (uo/vo), SST, Salinitas, Klorofil-a (bio dataset)
  2. ERA5   → Angin (u10/v10); fallback Open-Meteo (tanpa akun)
...
def fetch_cmems(user: str, password: str) -> dict:
    ...
    # ── A. Data fisik (arus, SST, salinitas) ──────────────────────────────
    ERDDAP_PHY_DATASETS = [
    ...
    for dataset_id in ERDDAP_PHY_DATASETS:
        ...
        try:
            resp = _get(url + query, auth=(user, password), timeout=25)
    ...
    # Coba D-2 sampai D-8 karena NRT butuh waktu proses 2-3 hari
    date_candidates = [
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if len(v):
                        ...
                        print(
                            f"[CMEMS] ✓ Chl-a dari {dataset_id} "
                            f"({date_str[:10]}): {result['chla']:.3f} mg/m³"
                        )
                        break
            except Exception as e:
    ...
    # ── C. WMS fallback — verifikasi koneksi ─────────────────────────────
    if not result:
        try:
            wms_url = (
...
#    Klorofil-a sepenuhnya dari CMEMS. Fungsi ini dipertahankan
#    agar tidak memecah import lama.
# =============================================================================
...
def build_realtime_dataframe(cmems_user: str, cmems_pass: str,
    ...
    if r["ok"] and r["data"]:
        ...
        status["CMEMS"] = True
        any_ok = True
        chla_src = "bio dataset" if "chla" in r["data"] else "klimatologi"
        print(f"[LAUTAN] ✓ CMEMS OK (Chl-a: {chla_src})")
    else:
    ...
    df_out["Ocean_Health_Index"] = (
        0.25 * _norm(df_out["do"],        4.5,  7.5) +
        0.20 * _norm(df_out["ph"],        7.9,  8.4) +
        0.20 * _norm(df_out["chla"],      0.05, 0.8) +
        0.15 * _norm(df_out["salinitas"], 32.0, 36.5) +
        0.20 * (1 - _norm(df_out["gelombang"], 0.2, 2.5))
    ) * 100

    df_out["Fisheries_Index"] = (
        0.35 * _norm(df_out["chla"],          0.05, 0.8) +
        0.25 * _norm(df_out["do"],            4.5,  7.5) +
        0.20 * _norm(df_out["current_speed"], 0.0,  0.25) +
        0.20 * (1 - _norm(df_out["gelombang"], 0.2, 2.5))
    ) * 100

    active = ["CMEMS", "ERA5/Open-Meteo", "BMKG"]
    n_ok   = sum(status[k] for k in active)
    print(f"\n[LAUTAN] Selesai: {n_ok}/{len(active)} API aktif berhasil")
